...
const App = () => {
    ...
    let filteredData = data.filter((item) => {
        return (
            moment(item.startDate, dateFormat).toDate() < moment(item.endDate, dateFormat).toDate()
        );
    });
    ...
    if (startDate && endDate) {
        filteredData = filteredData.filter((item) => {
            return (
                (moment(item.startDate, dateFormat).toDate() > startDate._d &&
                    moment(item.startDate, dateFormat).toDate() < endDate._d) ||
                (moment(item.endDate, dateFormat).toDate() > startDate._d &&
                    moment(item.endDate, dateFormat).toDate() < endDate._d)
            );
        });
    }
